...
import { normalizeContent } from "../../mcp-server/ingest/normalizer.js";
import { ingestDocument } from "../../mcp-server/ingest/pipeline.js";

export async function runExpandedFeaturesTests() {
  ...
  try {
    ...
    mammoth.convertToMarkdown = async () => {
    ...
    console.log("✅ ALL EXPANDED PARSING & DOCUMENT RETRIEVAL TESTS PASSED!");
  } finally {
    if (existsSync(TEST_DIR)) {
      try {
...
if (process.argv[1] && process.argv[1].endsWith("expanded_features.test.js")) {
  runExpandedFeaturesTests().catch((err) => {
