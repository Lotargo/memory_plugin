...
## CPU-First Design Philosophy & Model Selection

### Why Pure CPU Execution? (No GPU / WebGPU Dependency)
`@lotargo/memory_plugin` was intentionally engineered as a **pure CPU-first, zero-overhead extension**.

During early R&D, WebGPU (`onnxruntime-web` / WGPU) and GPU execution providers were evaluated and partially prototyped. However, GPU execution requires heavy OS-dependent graphics drivers, massive native binary bundles (CUDA / Vulkan toolchains), and platform-specific native compilation. This breaks the core mission of an instant, lightweight, zero-config plugin.

By focusing on optimized CPU inference:
- Installs in seconds with **zero native C++ compilation (`node-gyp`)**.
- Runs reliably across any Linux, macOS (Apple Silicon / Intel), or Windows environment.
- Operates with minimal CPU and memory footprint directly alongside your active IDE.

### Model Choices in CLI: Default Micro vs Heavy Models
- **`Xenova/multilingual-e5-small` (Default)**: Optimized specifically for CPU execution (~47M parameters, ~120 MB quantized). Delivers sub-millisecond vector inference and state-of-the-art accuracy with negligible RAM usage.
- **Larger Models (`multilingual-e5-large`, `bge-m3`)**: Available in `memory-cli` as **experimental options**. These models are designed for high-performance workstations equipped with powerful multi-core CPUs and abundant RAM. For standard daily coding, the default micro model is recommended.
