...
async function removeConflictingLocalDocument(db, doc) {
  ...
  const localUpdated = Number(conflict.updated_at || 0);
...
async function applyRemoteDocumentBundle(db, bundle) {
  ...
  if (local && localUpdated > remoteUpdated) {
    return { action: "local_newer", blobHash: doc.blob_hash };
  }
  if (local && localUpdated === remoteUpdated && local.blob_hash === doc.blob_hash && local.path === doc.path) {
    return { action: "unchanged", blobHash: doc.blob_hash };
  }
  ...
  try {
    ...
    if (conflictResult?.blocked) {
      ...
      return { action: "path_conflict_local_newer", blobHash: doc.blob_hash };
    }
    orphanCandidate = conflictResult?.conflict?.blob_hash || null;

    if (local) {
      await clearLocalDocumentRelations(db, doc.id);
    }
    ...
    const scopes = bundle.document_scopes.length
      ? bundle.document_scopes
      : [{ scope_key: "global", created_at: doc.created_at || Date.now() }];
    for (const scope of scopes) {
      await db.prepare(
        "INSERT OR IGNORE INTO document_scopes (doc_id, scope_key, created_at) VALUES (?, ?, ?);"
      ).run(doc.id, scope.scope_key || "global", scope.created_at || Date.now());
    }
    ...
    for (const medium of bundle.medium_chunks) {
      ...
      `).run(
        ...
        doc.id,
        medium.content || "",
        medium.block_type || "paragraph",
        Number(medium.token_count || 0),
        medium.created_at || Date.now()
      );
    }
    ...
    for (const chunk of bundle.micro_chunks) {
      ...
      `).run(
        ...
        chunk.medium_id || null,
        chunk.retrieval_policy || "micro_chunk",
        chunk.policy_source_id || null
      );
      await db.prepare(
        "INSERT INTO micro_chunks_fts (id, content, breadcrumbs) VALUES (?, ?, ?);"
      ).run(chunk.id, chunk.content || "", sectionBreadcrumbs.get(chunk.section_id) || "");
    }
  ...
  if (local?.blob_hash && local.blob_hash !== doc.blob_hash) {
    await cleanupOrphanBlob(db, local.blob_hash);
  }
  return { action: local ? "updated" : "pulled", blobHash: doc.blob_hash };
}

/**
 * Merge cloud RAG state into the local SQLite copy in hybrid-sync mode.
 *
 * This is intentionally non-destructive for local-only documents: missing cloud
 * rows are not treated as deletion tombstones. Cloud/newer documents are restored
 * by stable ID/path and raw blobs are materialized from `rag_blobs` when available.
 */
export async function pullRagFromCloud(db) {
  if (!db || db.mode !== "hybrid-sync" || !db.cloudClient) {
    return { skipped: true };
  }
